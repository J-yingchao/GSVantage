...
export function leafletMap({
  ...
  if (!map) {
    ...
    info.update = function (props) {
      this._div.innerHTML = '<h4>Detroit Census Tract</h4>' + (props ?
  ...
  const detroitWGS84 = convertToWGS84GeoJSON(detroitGeo, fromGeo);
  ...
  // Color scale
  const acresArray = detroitWGS84.features.map(d => d.properties?.avg_staleness || 0);
  ...
  geojsonLayer = L.geoJSON(detroitWGS84, {
    style: feature => ({
      color: '#808080',
      weight: 1,
      opacity: 0.8,
