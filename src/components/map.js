...
export function leafletMap({
  ...
  if (!map) {
    ...
    info.update = function (props) {
      this._div.innerHTML = '<h4>Seattle Block Group</h4>' + (props ?
  ...
  const seattleWGS84 = convertToWGS84GeoJSON(seattleGeo, fromGeo);
  ...
  // Color scale
  const acresArray = seattleWGS84.features.map(d => d.properties?.avg_staleness || 0);
  ...
  geojsonLayer = L.geoJSON(seattleWGS84, {
    style: feature => ({
      color: '#808080',
      weight: 1,
      opacity: 0.8,
