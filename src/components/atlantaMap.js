...
export function leafletMap({
  ...
  if (!map) {
    ...
    info.update = function (props) {
      this._div.innerHTML = '<h4>Atlanta Census Tract</h4>' + (props ?
        `<b>${props.NAMELSAD}</b><br/>
         Avg_Staleness: ${props.avg_staleness || 'N/A'}<br/>
  ...
  const atlantaWGS84 = convertToWGS84GeoJSON(atlantaGeo, fromGeo);
  ...
  // Color scale
  const acresArray = atlantaWGS84.features.map(d => d.properties?.avg_staleness || 0);
  ...
  geojsonLayer = L.geoJSON(atlantaWGS84, {
    style: feature => ({
      color: '#808080',
      weight: 1,
      opacity: 0.8,
